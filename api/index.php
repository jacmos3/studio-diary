...
try {
  ...
  if (preg_match('#^/days/(\d+)/tracks$#', $path, $m) && $method === 'POST') {
    $dayId = (int)$m[1];
    $result = studio_store_uploaded_tracks($dayId, $_FILES);
    studio_json_response(201, $result);
  }
  ...
  if (preg_match('#^/tracks/(\d+)$#', $path, $m) && $method === 'DELETE') {
    $trackId = (int)$m[1];
    $day = studio_delete_track($trackId);
    studio_json_response(200, ['ok' => true, 'day' => $day]);
  }

  studio_json_response(404, ['error' => 'Not found']);
} catch (Throwable $error) {
  studio_json_response(500, ['error' => $error->getMessage()]);
}
